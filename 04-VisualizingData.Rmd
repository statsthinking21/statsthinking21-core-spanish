...
```{r srbLeak, echo=FALSE,fig.cap="Imagen del sólido propulsor de cohete fugando combustible, segundos antes de la explosión. By NASA (Great Images in NASA Description) [Public domain], via Wikimedia Commons",fig.height=3,out.height='20%'}
...
```{r challengerTemps, echo=FALSE,fig.cap="Replanteo de los datos del índice de daños de Tufte. La línea muestra la tendencia en los datos y el parche sombreado muestra las temperaturas proyectadas para la mañana del lanzamiento.",fig.width=8,fig.height=4,out.height='50%'}
...
2. La segunda gráfica muestra las barras con todos los puntos de datos (data points) sobrepuestos - esto hace un poco más claro que la distribución por altura entre hombres y mujeres están sobrepuestos, pero aún es difícil ver por la grande cantidad de puntos de datos.  
...
Se han escrito muchos libros acerca de la visualización efectiva de los datos. Hay algunos principios en los que la mayoría de lxs autorxs están de acuerdo, mientras que otros son más polémicos.  Aquí resumimos algunos de los principios fundamentales; si quieres aprender más, entonces algunos buenos recursos están enlistados en la sección de *Lecturas sugeridas* al final del capítulo. 
...
2. Si vemos la gráfica en el panel B, puede que desconfiemos -- pocas veces datos reales siguen un patrón preciso. 
...
4. Los datos en el panel D nos muestran, aparentemente que la relación entre las dos variables es solamente causada por un inviduo, al que nos referiremos como *atípico* (outlier) porque cae muy lejos del patrón del resto del grupo. Debería de ser claro que probablemente no queremos sacar muchas conclusiones de un efecto guiado por un solo punto de los datos. Esta figura resalta por qué es *siempre* importante mirar los datos sin procesar antes de confiar demasiado en cualquier resumen de los datos.
...
### Maximizar la porporción datos/tinta (data/ink ratio) 
<!-- Edward Tufte has proposed an idea called the data/ink ratio:-->
Edward Tufte propuso la idea llamada *proporción datos/tinta* (*data/ink ratio*)
$$
...
<!-- It's especially common to see presentations of data in the popular media that are adorned with lots of visual elements that are thematically related to the content but unrelated to the actual data.  This is known as *chartjunk*, and should be avoided at all costs. One good way to avoid chartjunk is to avoid using popular spreadsheet programs to plot one's data.  For example, the chart in Figure \@ref(fig:chartJunk) (created using Microsoft Excel) plots the relative popularity of different religions in the United States.  There are at least three things wrong with this figure:
...
```{r crimePlotAxes,echo=FALSE,fig.cap="Datos de crimen de 1990 a 2014 graficados con el tiempo. Los paneles A y B muestran los mismos datos pero con diferentes rangos de eje. Datos obtenidos de https://www.ucrdatatool.gov/Search/Crime/State/RunCrimeStatebyState.cfm",fig.width=8,fig.height=4,out.height='50%'}
...
```{r bodyTempAxis,echo=FALSE,fig.cap="Temperatura corporal a lo largo del tiempo, graficado con o sin el punto cero en el eje Y.",fig.width=8,fig.height=4,out.height='50%'}
...
```{r barCharLieFactor, echo=FALSE,fig.cap="Dos gráficos de barra con la asociación del factor de engaño ",fig.width=8,fig.height=4,out.height='50%'}
...
## Acomodando las limitaciones humanas 
...
```{r pieChart,echo=FALSE,fig.cap="Un ejemplo de una gráfica de pie, enfatizando la dificultad en comprendiendo el volumen relaticvo de las diferentes rebanadas de pie.", fig.width=6,fig.height=6,out.height='50%'}
...
```{r gasPrices,echo=FALSE,message=FALSE, warning=FALSE,fig.cap="El precio de la gasolina en Estados Unidos de 1930 a 2013 (obtained from http://www.thepeoplehistory.com/70yearsofpricechange.html) con o sin la corrección para inflación (based on Consumer Price Index).",fig.width=4,fig.height=4,out.height='50%'}
