...
kable(PhysActive_table, digits=3) #, label="PhysActiveTable",caption="Distribución de frecuencias de la variable PhysActive")
```
...
Esto nos deja ver que el `r formatC(I(PhysActive_table_filtered %>% subset(PhysActive=='No') %>% dplyr::select(Percentage) %>% pull()), digits=1, format='f')` por ciento de los individuos en la muestra NHANES dijo "No" y el `r formatC(I(PhysActive_table_filtered %>% subset(PhysActive=='Yes') %>% dplyr::select(Percentage) %>% pull()), digits=1, format='f')` por ciento dijo "Sí".
